...
export async function lspRequest(
  ...
  } else if (client.name === "lspts") {
    return await lspTSRequest(denops, client, method, params);
  } else {
...
async function lspTSRequest(
  denops: Denops,
  client: Client,
  method: Method,
  params: unknown,
): Promise<LspResult> {
  return await denops.dispatch("lspts", "request", client.id, method, params);
}
